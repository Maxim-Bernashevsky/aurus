...
class Order extends Component {
  ...
  componentWillMount(){
    ...
    fb.on("value", snapshot => {
        ...
        this.orderData = snapshot.val().orders[orderID];
      },
  ...
  render() {
    ...
    if(!this.orderData) return (this.headBlock());

    const {color, interior, options, model} = this.orderData;
        ...
        <table className="totalPrice">
          <tbody>
            <tr>
              ...
              <td className="orderInfoValue">{getTextPrice(getTotalPrice(this.orderData))} <sub>&#8381;</sub></td>
            </tr>
          </tbody>
        </table>
